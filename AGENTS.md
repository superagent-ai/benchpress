...
`owasp` and `bountybench` were both unblocked once detect-only mode shipped (see `src/benchmarks/owasp/adapter.ts`
and `src/benchmarks/bountybench/adapter.ts`). None of the three appear below anymore.
...
Do not implement scientific benchmark bodies until the dependency row is satisfied in autobrin-flue. CyberGym is a documented, issue-scoped exception (superagent-ai/benchpress#16): `setup()`/`listTasks()`/`standUpTarget()` don't need the blocked capabilities (vendored task metadata, real dockerized pre-/post-patch target stand-up), so they're implemented for real against a representative subset; only `score()` throws `NotImplementedBenchmarkError` until autobrin-flue#180 (PoC-gen skill) and #181 (differential oracle) land. `owasp`'s `score()` (superagent-ai/benchpress#30) grades detect-only mode's confirmed/rejected verdict (`TargetHandle.detectOnly`, read by `buildRepoPayload` in `src/contenders/autobrin.ts`) against `expectedresults-1.2.csv` ground truth into TP/FP/FN/TN, reported via the existing generic `youdenIndex()`/`aggregateOracleScores()` pipeline (`src/matrix/report.ts`) -- no benchmark-specific reporting code needed.

**`bountybench` is fully unblocked too** (autobrin-flue#182 merged to `staging`, the same `TargetHandle.detectOnly` field owasp reads): Detect, Exploit, and Patch lanes' `score()` all do real work now. Detect needs no verifier at all (every task in the curated manifest is a known-vulnerable target, so the contender's `detectOnly` confirmed/rejected verdict is compared directly against that ground truth). Patch takes the contender's own `proposed_patch.diff`, applies it to a fresh codebase copy, and re-verifies the vulnerability is gone with a real per-bounty check (`src/benchmarks/bountybench/patchVerifiers.ts`) — wired for `zipp` bounty 0 only today, the same narrow-by-design coverage as Exploit's `lunary` bounty 0 verifier; a claim that confirms the vulnerability but proposes no patch (e.g. PITHOS, whose `TRIAGE.json` has no patch field) gets an explicit `not_scored` result, not a crash or a fake pass — Patch lane is autobrin-only by design (superagent-ai/benchpress#31). See `src/benchmarks/bountybench/README.md`.
